...
import type { VscodeTaskStatus } from "./vscodeTask.js";

export type InitOptions = {
  installVscodeTask?: boolean;
};

export type InitResult = {
  created: string[];
  skipped: string[];
  vscodeTaskStatus: VscodeTaskStatus | "skipped";
};
...
export async function initProject(rootDir: string, options: InitOptions = {}): Promise<InitResult> {
  const contextPaths = getContextPaths(rootDir);
  const created: string[] = [];
  const skipped: string[] = [];
  let vscodeTaskStatus: VscodeTaskStatus | "skipped" = "skipped";
  ...
  if (options.installVscodeTask !== false) {
    const taskUpdate = await ensureVscodeAutoTask(rootDir);
    vscodeTaskStatus = taskUpdate;
    if (taskUpdate === "created" || taskUpdate === "updated") {
      created.push(".vscode/tasks.json");
    } else {
      skipped.push(".vscode/tasks.json");
    }
  }

  return { created, skipped, vscodeTaskStatus };
}
